...
class ScriptImporterExporter < PluginBase
  ...
  def on_exit    
    ...
    if (file_modified_since?($INPUT_DIR + "Scripts.#{$DATA_TYPE}", $STARTUP_TIME)) and (File.exists?($SCRIPTS_DIR + "/" + $EXPORT_DIGEST_FILE) || file_modified_since?($INPUT_DIR + "EditorScripts.#{$DATA_TYPE}", $STARTUP_TIME)) and (File.exists?($SCRIPTS_DIR + "/" + $EXPORT_DIGEST_FILE))
    ...
    scripts.each_index do |i|
      if digest[i][2].upcase != "EMPTY"
        ...
        File.open($OUTPUT_DIR + "Scripts/" + digest[i][2], File::WRONLY|File::CREAT|File::TRUNC|File::BINARY) do |outfile|
    ...
    scripts.each_index do |i|
      if digest[i][2].upcase != "EMPTY"
        inflate_start_time = Time.now
        File.open($OUTPUT_DIR + "EditorScripts/" + digest[i][2], File::WRONLY|File::CREAT|File::TRUNC|File::BINARY) do |outfile|
          outfile << Zlib::Inflate.inflate(scripts[i][2])
        end
        num_exported += 1
        inflate_elapsed_time =  Time.now - inflate_start_time
